  ...
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        <h2 className="text-2xl font-black text-stone-900 mb-8">数据概览</h2>

        {/* Stats Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          <div className="bg-white rounded-2xl p-6 border border-stone-100 shadow-sm">
            <div className="text-sm font-bold text-stone-400 mb-2">总测评数</div>
            <div className="text-4xl font-black text-stone-900">{stats?.totalAssessments || 0}</div>
          </div>
          <div className="bg-white rounded-2xl p-6 border border-stone-100 shadow-sm">
            <div className="text-sm font-bold text-stone-400 mb-2">今日新增</div>
            <div className="text-4xl font-black text-emerald-600">{stats?.todayAssessments || 0}</div>
          </div>
          <div className="bg-white rounded-2xl p-6 border border-stone-100 shadow-sm">
            <div className="text-sm font-bold text-stone-400 mb-2">可用兑换码</div>
            <div className="text-4xl font-black text-orange-600">{stats?.unusedCodes || 0}</div>
          </div>
          <div className="bg-white rounded-2xl p-6 border border-stone-100 shadow-sm">
            <div className="text-sm font-bold text-stone-400 mb-2">已使用兑换码</div>
        ...
        <div className="bg-white rounded-2xl border border-stone-100 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-stone-100 flex justify-between items-center">
            <h3 className="font-bold text-stone-900">最近测评</h3>
          ...
          <div className="divide-y divide-stone-100">
            {recentAssessments.length === 0 ? (
              ...
              recentAssessments.map((assessment) => (
                <div key={assessment.id} className="px-6 py-4 flex items-center justify-between hover:bg-stone-50">
                  <div>
                    <div className="font-bold text-stone-900">{assessment.code}</div>
